...
"""Nave"""
condicionesIniciales=[-20.,         -16.11339198, 709.87292278]
vship=vector(condicionesIniciales[0], condicionesIniciales[1],0) #Velocidad Inicial nave
ANG=pi/2
...
Mercury = sphere(pos=vector(rmerc*ca,rmerc*sa,0),
                 radius=0.05, color=color.red,make_trail=False, interval=10)
Mercury.mass = mmerc
...
Jupyter = sphere(pos=vector(rjup*ca,rjup*sa,0),
                 radius=0.3, color=color.green,make_trail=False, interval=10)
Jupyter.mass = mjup
Jupyter.v = vector(vjup*cca, vjup*sca, 0)

Saturn = sphere(pos=vector(rsat*ca,rsat*sa,0),
                radius=0.25, color=color.green,make_trail=False, interval=10)
Saturn.mass = msat
Saturn.v = vector(vsat*cca, vsat*sca, 0)

Uranus = sphere(pos=vector(rur*ca,rur*sa,0),
                radius=0.2, color=color.green,make_trail=False, interval=10)
Uranus.mass = mur
...
Ship = sphere(pos=vector(rship, 0, 0),
              radius=0.00001, color=color.orange,make_trail=True, interval=10)
Ship.mass = 3.214e-28
Ship.v = vship
...
while True:
    rate(500)
    if tiempo==int(condicionesIniciales[2]):
        body.pos=Earth.pos
    for body in [Mercury,Venus,Earth,Mars,Jupyter,Saturn,Uranus,Neptune,Ship]:
        if body == Ship and tiempo>=int(condicionesIniciales[2]):
            DS=body.pos-Sun.pos
